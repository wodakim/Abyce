...
import { Position, PrevPosition, Acceleration, VerletPoint, Constraint, PlayerTag } from './core/ecs/components';
...
function createCell(centerX: number, centerY: number, radius: number, segments: number, isPlayer: boolean = false) {
    ...
    world.getComponent(VerletPoint.name).add(centerEntity, [radius/2, 0.95, 0]); // Smaller visual radius for center
    ...
    for (let i = 0; i < segments; i++) {
        ...
        world.getComponent(VerletPoint.name).add(segment, [5, 0.9, 0]); // Radius 5 for membrane points

        // Constraint to Center
        const constraint1 = world.createEntity();
        world.getComponent(Constraint.name).add(constraint1, [centerEntity, segment, radius, 0.05]); // Soft stiffness
    ...
    for (let i = 0; i < segments; i++) {
        ...
        world.getComponent(Constraint.name).add(constraint2, [current, next, dist, 0.8]); // Stiff membrane
    }
}
...
// Spawn some dummy cells
for(let i=0; i<5; i++) {
    createCell(Math.random() * window.innerWidth, Math.random() * window.innerHeight, 30, 8, false);
}
...
loop.start();
console.log("ABYCE Phase 2 Started.");
